...
cdlisturl = 'https://y.qq.com/n/yqq/playlist/785829613.html'
print(cdlisturl)
...
header = {
    ...
    'Referer': 'https://y.qq.com/n/yqq/playlist/' + cdlistid + '.html', 'Host': 'c.y.qq.com'}
resp = requests.get(
    'https://c.y.qq.com/qzone/fcg-bin/fcg_ucc_getcdinfo_byids_cp.fcg?type=1&json=1&utf8=1&onlysong=0&disstid=' + cdlistid + '&format=jsonp&g_tk=' + cdlistid + '&jsonpCallback=playlistinfoCallback&loginUin=12888097&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0',
    headers=header)
...
# print j
# 设定需要下载的文件格式。分别为C400 m4a，M500 mp3 128k，M800 mp3 320k，A000 ape ,F000 Flac格式
filerate = "F000"
# 优先下载高品质 (320k mp3)
file_lowerrate = "M500"
# 替代下载品质（128k Mp3）
...
for n in range(0, len(mids)):
    resp2 = requests.get(
        ...
        songmids[n] + '&filename=M500' + mids[n] + '.mp3' + '&guid=6612300644')
...
for m in range(0, x):
    ...
    if (len(vkeys[m]) > 10):
        ...
        srcs = 'http://dl.stream.qqmusic.qq.com/' + filerate + strMediaMid[m] + filetype + '?guid=6612300644&vkey=' + \
               vkeys[m] + '&uin=0&fromtag=53'
        srcs_lowres = 'http://dl.stream.qqmusic.qq.com/' + file_lowerrate + strMediaMid[
            m] + filetype + '?guid=6612300644&vkey=' + vkeys[m] + '&uin=0&fromtag=53'
